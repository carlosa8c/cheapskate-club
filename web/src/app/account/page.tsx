...
export default async function Account({ searchParams }: { searchParams: Promise<{status?: string}> }) {
  ...
  const { data: profile, error } = await client.from("club_profiles").select("handle,display_name,sharing_enabled").eq("id", user.id).maybeSingle();
  const { status } = await searchParams;
  return <section className="prose"><p className="eyebrow">YOUR CORNER OF THE CLUB</p><h1>Make yourself<br/><em>at home.</em></h1>
