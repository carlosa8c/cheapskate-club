...
export default async function BuildPage({params,searchParams}:{params:Promise<{id:string}>;searchParams:Promise<{status?:string}>}){
 ...
 {build.show_usage&&<aside className="build-usage"><h2>The builder’s Club card</h2><p>Lifetime zero-cost tokens across their projects.</p><Link href={`/@${build.handle}`}><img src={`/api/cards/${build.handle}?download=1`} alt={`${build.display_name}’s lifetime usage card`} loading="lazy"/></Link></aside>}
