...
export async function POST(request: Request) {
 ...
 if(verified.message.action==="status") {
  const {data:installation,error:lookupError}=await client.from("club_installations").select("owner_id,pairing_id,sequence_number,previous_hash").eq("id",verified.message.installation_id).eq("public_key",verified.key).maybeSingle();
  if(lookupError) return Response.json({error:"Connection lookup unavailable."},{status:503});
  if(installation?.owner_id && installation.pairing_id) {
   const {data:profile,error:profileError}=await client.from("club_profiles").select("id,handle,display_name").eq("id",installation.owner_id).single();
   if(profileError || !profile) return Response.json({error:"Connection account unavailable."},{status:503});
   return Response.json({status:"connected",pairing_id:installation.pairing_id,account_id:profile.id,handle:profile.handle,name:profile.display_name,sequence:installation.sequence_number,previous_hash:installation.previous_hash},{headers:{"Cache-Control":"no-store"}});
  }
 }
