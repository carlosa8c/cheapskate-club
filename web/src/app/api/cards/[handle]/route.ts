...
export async function GET(request:Request,{params}:{params:Promise<{handle:string}>}){
 ...
 try{
  ...
  return new ImageResponse(createElement(ShareCard,{member,champion,square,date}),{fonts:[{name:'Club Sans',data:sans.buffer.slice(sans.byteOffset,sans.byteOffset+sans.byteLength) as ArrayBuffer,weight:400,style:'normal'},{name:'Club Serif',data:serif.buffer.slice(serif.byteOffset,serif.byteOffset+serif.byteLength) as ArrayBuffer,weight:400,style:'normal'}],width:square?1080:1200,height:square?1080:630,headers:{...headers,...(square?{'Content-Disposition':`attachment; filename="cheapskate-${handle}.png"`}:{})}});
 }catch{return new Response('Card unavailable. Please try again.',{status:503,headers});}
}
