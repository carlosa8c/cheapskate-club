...
export async function generateMetadata({params}:{params:Promise<{handle:string}>}):Promise<Metadata>{
 ...
 const title=`${member.display_name} · The Cheapskate Club`,description=`${member.tokens.toLocaleString('en-US')} zero-cost tokens & counting. Less bill. More brag.`;
...
export default async function MemberPage({params}:{params:Promise<{handle:string}>}) {
 ...
 return <section className="member-page"><Link href="/">← The leaderboard</Link><div className="member-heading"><div className="member-monogram" aria-hidden="true">{member.display_name.slice(0,1).toUpperCase()}</div><div><p className="eyebrow">MEET A FELLOW CHEAPSKATE</p><h1>{member.display_name}</h1><p>@{member.handle}</p></div></div><MemberStats member={member}/>{shareable&&<ShareActions handle={member.handle} url={`${clubOrigin()}/@${member.handle}`}/>}<div className="invite"><div><h2>Good company. Better ideas.</h2><p>There’s always room for another builder.</p></div><Link className="button" href="/community">Around the club →</Link></div></section>;
}
