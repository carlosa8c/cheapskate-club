...
export function BuildDetailView({
  ...
  const shareUrl = `${shareOrigin.replace(/\/$/, "")}/community/${build.id}`;
  const tweetText = encodeURIComponent(`${build.title} — showcase build on The Cheapskate Club`);
  ...
  return (
    <section className="build-detail">
      ...
      <p className="eyebrow">A FELLOW CHEAPSKATE MADE THIS</p>
      <h1>{build.title}</h1>
      <p className="lede">
        By{" "}
        {build.profile_public ? (
          <a href={`/@${build.handle}`} style={{ textDecoration: "underline" }}>
      ...
      {isOwn && (
        <details className="profile-settings" style={{ marginTop: "24px", padding: "16px", border: "1px dashed var(--line)", borderRadius: "8px" }}>
          ...
          <div style={{ marginTop: "12px", display: "flex", gap: "12px", alignItems: "center" }}>
            <a className="button" href={`/community/${build.id}/edit`}>
              Edit build →
            </a>
            <button
              type="button"
